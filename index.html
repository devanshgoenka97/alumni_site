<!DOCTYPE html>
<html lang="en">

	<head>
		<title>NIT DURGAPUR ALUMNI CELL</title>
		<meta charset="utf-8">
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<link href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" rel="stylesheet">
        <link href="https://fonts.googleapis.com/css?family=Acme" rel="stylesheet">
        <link href="css/font-awesome.min.css" rel="stylesheet">
        <link rel="stylesheet" href="css/layout.css">
        <link rel="stylesheet" href="css/content.css">
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
	</head>
    
    <body>
    	<header class="header container-fluid">
            <div class="row header-text">
            <div class="col-md-3">
            </div>
            <img class="logo col-md-2" src="img/logo.png"/>
            <div class="primary-header-text col-md-7">
            <h1 class="no-top-bottom-margin">
                ALUMNI CELL
            </h1>
    		<h2 class="no-top-bottom-margin">
                National Institute of Technology Durgapur
            </h2>
            <h3 class="no-top-bottom-margin">
                राष्ट्रीय प्रौद्योगिकी संस्थान दुर्गापुर
            </h3>
            <h5 class="no-top-bottom-margin">(An Institute of National Importance under Government of India, Ministry of Human Resource Development) 
               <br> Mahatma Gandhi Avenue, Durgapur 713209, West Bengal, INDIA</h5>
            </div>
            </div>
            <!-- <div class="row">
                <div class="secondary-header-text col-md-12 text-center">
                    <h2>Alumni Cell</h2>
                </div>
            </div> -->
        <nav class="navbar navbar-inverse">
            <div class="container-fluid no-left-right-padding">
            <ul class="nav navbar-nav">
                <li><a href="javascript:void(0);" id="home">Home</a></li>
                <li class="dropdown">
                    <a class="dropdown-toggle" data-toggle="dropdown" href="#">About Us</a>
                    <ul class="dropdown-menu">
                        <li><a href="javascript:void(0);" id="aims">Objectives</a></li>
                        <li><a href="javascript:void(0);" id="messagefromdean">Message from the Director and Dean(AOT&P)</a></li>
                        <li><a href="javascript:void(0);" id="meettheteam">Our Team</a></li>
                    </ul>
                </li>
                <li class="dropdown">
                    <a class="dropdown-toggle" data-toggle="dropdown" href="#">Our Alumni</a>
                    <ul class="dropdown-menu">
                        <li><a href="javascript:void(0);" id="alumnistats">Alumni Statistics</a></li>
                        <li><a href="javascript:void(0);" id="searchalumni">Search our Alumni</a></li>
                        <li><a href="javascript:void(0);" id="alumniregs">Alumni Registration</a></li>
                    </ul>
                </li>
                <li class="dropdown">
                    <a class="dropdown-toggle" data-toggle="dropdown" href="#">Events</a>
                    <ul class="dropdown-menu">
                        <li><a href="javascript:void(0);" id="popularlectures">Popular Lectures</a></li>
                        <li><a href="javascript:void(0);" id="beyondthewall">Beyond The Wall Lectures Series</a></li>
                        <li><a href="javascript:void(0);" id="studentalumnimeet">Student-Alumni Meet</a></li>
                    </ul>
                </li>
                <li><a href="javascript:void(0);" id="alumniawards">Alumni Awards</a></li>
                <li><a href="javascript:void(0);" id="givingback">Giving Back to Alma Mater</a></li>
                <li><a href="javascript:void(0);" id="alumniservices">Alumni Services</a></li>
                <li class="dropdown">
                    <a class="dropdown-toggle" data-toggle="dropdown" href="#">Students' Alumni Body</a>
                    <ul class="dropdown-menu">
                        <li><a href="javascript:void(0);" id="alumnistudentmentorship">Alumni-Student Mentorship Programme</a></li>
                        <li><a href="javascript:void(0);" id="iiiprogramme">Industry-Institute-Interaction Programme</a></li>
                        <li><a href="javascript:void(0);" id="studentvolunteers">Student Volunteers</a></li>
                    </ul>
                </li>
                <li><a href="#">Alumni Association(NITDAA)</a></li>
            </ul>
        </div>
        </nav>
    	</header>

        <section class="container-fluid content" id="fillable">
        </section>

        <footer class="footer container-fluid text-center foot">Copyright © 2017. All Rights Reserved. National Institute of Technology, Durgapur.
         </footer>
    </body>
    <script src="js/custom.js"></script>
</html>